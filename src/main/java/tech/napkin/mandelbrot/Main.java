...
public class Main { // todo: make oop or something i dunno
	...
	public static void main(String[] args) {
		...
		for (row = 0; row < (config.optimized ? config.height / 2 + 1 : config.height); ++row) {
			...
			final double imaginary_c = (row - config.height / 2.0) * 4.0 / config.width;
			...
			for (int col = 0; col < config.width; ++col) {
				...
				final double real_c = (col - config.width / 2.0) * 4.0 / config.width;
...
//					_image.setRGB(col, row, Color.getHSBColor((float) Math.toRadians(0.95 + 10.0 * (iterations + 1.0 - Math.log10(Math.log10(Math.abs(Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2))))) / Math.log10(2.0))), 0.6f, 1).getRGB());
		...
		System.out.println("Average pixel time: " + Math.round((totalColTime / (double) (config.width * config.height))) / 1000d + "µs");
